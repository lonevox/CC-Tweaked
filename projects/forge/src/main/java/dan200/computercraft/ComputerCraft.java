...
public final class ComputerCraft {
    ...
    public static IEventBus getEventBus() {
        var bus = eventBus;
        if (bus == null) throw new NullPointerException("Bus is not been initialised yet");
        return bus;
    }
    ...
    public static void registerNetwork(RegisterPayloadHandlerEvent event) {
        ...
        for (var type : NetworkMessages.getClientbound()) {
            ...
            registrar.play(forgeType.id(), forgeType.reader(), builder -> builder.client(
                (t, context) -> context.workHandler().execute(() -> t.payload().handle(ClientNetworkContext.get()))
            ));
        }
    }
